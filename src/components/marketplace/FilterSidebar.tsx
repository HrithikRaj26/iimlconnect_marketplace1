...
export function FilterSidebar({ filters, onChange, onApply, onReset, isMobile = false }: FilterSidebarProps) {
  return (
    <aside className={`w-full ${isMobile ? '' : 'rounded-xl border border-gray-200 bg-white p-5 shadow-card'}`}>
      {!isMobile && (
      ...
      <div className={`mt-4 space-y-2 ${isMobile ? 'sticky bottom-0 bg-white dark:bg-gray-900 py-4 border-t border-gray-100 dark:border-gray-800 pb-[env(safe-area-inset-bottom)]' : ''}`}>
